...
1. In `app_component.dart`,
   import and then add `const ClassProvider(HeroService)` to the `providers` list
...
<?code-excerpt "lib/src/route_paths_3.dart (hero)" replace="/hero\b/[!$&!]/g" title?>
```
  final idParam = 'id';
...
<?code-excerpt "lib/src/routes_3.dart (hero)" replace="/_hero\b/[!$&!]/g" title?>
```
...
<img class="image-display" src="{% asset_path 'ng/devguide/router/selected-hero.png' %}" alt="Selected hero" width="250px">
